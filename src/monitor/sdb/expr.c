...
void dump_tokens(int begin, int end)
{
	printf("begin: %d, end: %d\n", begin, end);
	for (int i = begin; i <= end; i++) {
		printf("token[%d], type: %d, char: %s\n", i, tokens[i].type, tokens[i].str);
	}
}
		...
		*success = false;
		...
		for (int i = begin; i <= end; i++) {
			...
			case ')':
				match--;
				break;
			case '+': case '-': case '*': case '/': case TK_DEREF: case TK_EQ: case TK_NEQ:
				if (match == 0 && (((type == 0 || type == '*' || type == '/' || type == TK_DEREF || type == '+' || type == '-') && (tokens[i].type == TK_EQ || tokens[i].type == TK_NEQ || tokens[i].type == '+'      || tokens[i].type == '-')) ||
						   ((type == 0 || type == '*' || type == '/' || type == TK_DEREF)                               && (tokens[i].type == '+'   || tokens[i].type == '-'    || tokens[i].type == '*'      || tokens[i].type == '/')) || 
						   ((type == 0 || type == TK_DEREF)                                                             && (tokens[i].type == '*'   || tokens[i].type == '/'    || tokens[i].type == TK_DEREF                         )) ||
